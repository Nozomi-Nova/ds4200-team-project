...
// Load data and render the line chart
d3.json("../data/sector_yearly_actual_ratings.json").then(data => {
    console.log("✅ Loaded actual credit ratings:", data);
    ...
    // Create the SVG container
    const svg = d3.select("#lineChartActual")
        .append("svg")
        .attr("width", width + margin.left + margin.right)
        .attr("height", height + margin.top + margin.bottom)
        .append("g")
    ...
    const line = d3.line()
        .x(d => xScale(new Date(d.Year, 0)))
        .y(d => yScale(d['Average Rating']))
        .curve(d3.curveBasis); // Smooth curve

    // Set the domains for scales
    xScale.domain(d3.extent(data, d => new Date(d.Year, 0)));
    yScale.domain([1, 22]); // Assuming 'AAA' = 1 and 'D' = 22
    colorScale.domain(nestedData.map(d => d[0]));
    ...
    svg.append("g")
        ...
        .attr("text-anchor", "middle")
        .attr("fill", "black")
        .text("Average Rating (Higher is Better)");
    ...
    svg.append("text")
        ...
        .style("font-size", "16px")
        .style("font-weight", "bold")
        .text("Sector-Based Trends in Actual Credit Ratings");

    // Add legend with checkboxes for filtering sectors
    const legend = d3.select("#lineChartActualControls")
        .style("display", "flex")
        .style("flex-wrap", "wrap")
        .style("margin-top", "30px");
    ...
    nestedData.forEach((d, i) => {
        ...
        container.append("label")
            .attr("for", `actual-sector-${i}`)
            .style("margin-left", "5px")
            .text(d[0]);
    });

    console.log("✅ Actual Ratings Line Chart Rendered");
}).catch(error => {
    console.error("❌ Error loading actual ratings data:", error);
});
...
d3.json("../data/actual_rating_feature_importance.json").then(data => {
    console.log("✅ Loaded actual rating feature importance data:", data);
    createActualRadarChartControls(data);
}).catch(error => {
    console.error("❌ Error loading actual radar chart data:", error);
});

function updateActualRadarChart(data, selectedSectors) {
    const topFeatures = ["Debt/Equity Ratio", "Current Ratio", "EBIT Margin", "Operating Margin", "ROA - Return On Assets"];

    const radarWidth = 850;
    const radarHeight = 750;
    const radius = Math.min(radarWidth, radarHeight) / 2 - 90;

    const radarSvg = d3.select("#actualRadarChart")
        .html("")
        .append("svg")
        .attr("width", radarWidth)
        .attr("height", radarHeight)
    ...
    const radialScale = d3.scaleLinear()
        .domain([-2.5, 2.5]) // Adjust if needed
        .range([0, radius]);
    ...
    selectedSectors.forEach(sector => {
        ...
        sectorData.push(sectorData[0]); // Close path
...
function createActualRadarChartControls(data) {
    const sectors = Object.keys(data);
    const controlsDiv = d3.select("#actualRadarChartControls").html("");
    ...
    sectors.forEach(sector => {
        ...
        control.append("input")
            ...
            .on("change", () => {
                ...
                updateActualRadarChart(data, selectedSectors);
            });

        control.append("label")
            .attr("for", `actual-radar-${sector}`)
            .style("margin-left", "5px")
            .text(sector);
    });

    // Preselect the first 3 sectors
    controlsDiv.selectAll("input").property("checked", (_, i) => i < 3);

    const selected = sectors.slice(0, 3);
    updateActualRadarChart(data, selected);
}
